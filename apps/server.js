...
export class ContainerKitMCPServer {
  ...
  async shutdown() {
    ...
    try {
      ...
      if (this.services.docker) {
        closePromises.push(this.services.docker.close?.());
      }

      if (this.services.kubernetes) {
        closePromises.push(this.services.kubernetes.close?.());
      }

      if (this.services.session) {
        closePromises.push(this.services.session.close?.());
      }

      if (this.services.ai) {
        closePromises.push(this.services.ai.close?.());
      }

      await Promise.allSettled(closePromises.filter(Boolean));
  ...
  async waitForActiveRequests() {
    // Implement tracking of active requests if needed
    // For now, just a brief delay to allow requests to complete
