...
import { WorkflowExecutionResult } from './orchestrator.js'
...
export class WorkflowManager extends EventEmitter {
  ...
  registerWorkflow(
    ...
    promise
      ...
      .catch(error => {
        execution.status = execution.abortController?.signal?.aborted === true ? 'aborted' : 'failed'
  ...
  listWorkflows(filter?: {
    ...
    return workflows.map(w => {
      const now = w.status === 'running' ? Date.now() :
        (w.result ? w.startTime + (w.result.duration || 0) : Date.now())
  ...
  abortWorkflow(sessionId: string): boolean {
    ...
    if (!workflow) {
      this.logger.warn({ sessionId }); // Fixed logger call
      return false
    }

    if (workflow.status !== 'running') {
      this.logger.warn({ sessionId, status: workflow.status }); // Fixed logger call
      return false
    }
    ...
    if (workflow.abortController) {
      ...
      this.logger.info({ sessionId, workflowId: workflow.workflowId }); // Fixed logger call
      return true
    }
  ...
  getMetrics(): WorkflowMetrics {
    ...
    const averageDuration = finishedWorkflows.length > 0 ?
      Math.round(totalDuration / finishedWorkflows.length) : 0
  ...
  getStatusSummary(): {
    ...
    return {
      ...
      queuedWorkflows: 0, // We don't have a queue yet
      recentCompletions,
      recentFailures,
      systemLoad
    }
  ...
  private archiveWorkflow(sessionId: string): void {
    ...
    if (this.completedWorkflows.length > this.maxCompletedHistory) {
      this.completedWorkflows.shift(); // Remove oldest
    }
  ...
  private cleanupOldWorkflows(): void {
    ...
    if (cleaned > 0) {
      this.logger.debug({ cleaned, remaining: this.completedWorkflows.length }); // Fixed logger call
    }
  }
  ...
  shutdown(): void {
    ...
    if (activeWorkflows.length > 0) {
      this.logger.info({ count: activeWorkflows.length }); // Fixed logger call
