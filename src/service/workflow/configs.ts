...
import type { DockerBuildResult, WorkflowState } from '../../domain/types/index.js'
import { WorkflowConfig } from './orchestrator.js'
import { mapWorkflowStateProperties } from './property-mappers.js'
...
function hasValidImageId(state: WorkflowState): boolean {
  return (state.build_result as DockerBuildResult)?.imageId != null && (state.build_result as DockerBuildResult)?.imageId.trim() !== ''
}
...
export const CONTAINERIZATION_WORKFLOW: WorkflowConfig = {
  ...
  steps: [
    ...
    },
    {
      ...
      paramMapper: (state, sessionId) => {
        const mapped = mapWorkflowStateProperties(state)
        const buildResult = (state.build_result as DockerBuildResult) ??
          (mapped.buildResult as DockerBuildResult)
        return {
          session_id: sessionId,
