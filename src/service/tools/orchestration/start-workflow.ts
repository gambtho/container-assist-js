...
export async function startWorkflowHandler(
  ...
  const { sessionService, workflowOrchestrator, workflowManager, progressEmitter, logger } = deps
  ...
  try {
    ...
    const workflowPromise = workflowOrchestrator.executeWorkflow(
      workflowConfig,
      sessionId,
      {
        repo_path: validated.repo_path,
 ...
 */
async function validateInput(input: StartWorkflowInput, logger: Logger): Promise<{
  repo_path: string
  workflow_type: string
  session_id?: string
  automated: boolean
  ...
  try {
    ...
    throw new Error(`Invalid repository path: ${repo_path} - ${error instanceof Error ? error.message : error}`)
  }

  // Validate workflow type
  const validWorkflowTypes = ['full', 'containerization', 'build-only', 'build', 'deploy-only', 'deploy', 'quick']
  if (!validWorkflowTypes.includes(workflow_type)) {
    throw new Error(`Invalid workflow type: ${workflow_type}. Valid types: ${validWorkflowTypes.join(', ')}`)
  }
