/**
 * Deploy Application Tool - TypeScript Implementation
...
import { Result, ok, fail, ErrorCode, DomainError, KubernetesManifest, KubernetesDeploymentResult } from '../../../domain/types/index.js'
import { executeWithRetry, getDeploymentErrorSuggestions } from '../error-recovery.js'
import type { ToolHandler, ToolContext } from '../tool-types.js'

// Input schema
const DeployApplicationInput = z.object({
...
async function loadManifests(manifestsPath: string): Promise<KubernetesManifest[]> {
  ...
  for (const file of yamlFiles) {
    ...
    try {
      ...
      manifests.push(...docs.filter((d: KubernetesManifest | null): d is KubernetesManifest => d != null && d.kind != null)) // Filter out null docs
    } catch (error) {
...
async function rollbackDeployment(
  ...
  if (kubernetesService) {
    for (const resource of deployed.reverse()) { // Delete in reverse order
      try {
        if ('delete' in kubernetesService) {
          await (kubernetesService as any).delete(resource, namespace)
        }
        logger.info(`Rolled back ${resource}`)
      } catch (error) {
        logger.error(`Failed to rollback ${resource}`, { error })
      }
    }
  }
}
...
async function waitForDeployment(
  ...
  while (Date.now() - startTime < timeout * 1000) {
    if ('getStatus' in kubernetesService) {
      const status = await (kubernetesService as any).getStatus(
 ...
 */
export const deployApplicationHandler: ToolHandler<DeployInput, DeployOutput> = {
  ...
  async execute(input: DeployInput, context: ToolContext): Promise<Result<DeployOutput>> {
    ...
    try {
      ...
      logger.info(`Loaded ${manifests.length} manifests`, {
        ...
        for (const deployment of deployments) {
          ...
          if (!ready) {
            ...
            if (rollbackOnFailure) {
              ...
              return fail(new DomainError(
                ErrorCode.TIMEOUT,
                `Deployment timeout and was rolled back`
              ))
            }
          }
        }
