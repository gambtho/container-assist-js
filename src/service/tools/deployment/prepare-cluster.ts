/**
 * Prepare Kubernetes Cluster Tool
 * Validates and prepares Kubernetes cluster for deployment
 */

import { z } from 'zod'
import { Result, ok, fail } from '../../../domain/types/result.js'
import { ErrorCode, InfrastructureError } from '../../../domain/types/errors.js'
 ...
 */
export const prepareClusterHandler: ToolHandler<PrepareClusterInput, PrepareClusterOutput> = {
  ...
  async execute(input: PrepareClusterInput, context: ToolContext): Promise<Result<PrepareClusterOutput>> {
    const { logger, sessionService, progressEmitter, kubernetesService: _kubernetesService } = context
    ...
    try {
      ...
      const clusterInfo = await simulateClusterPreparation(
 ...
 */
async function simulateClusterPreparation(
  context: string,
  namespace: string,
  dryRun: boolean,
  validateOnly: boolean,
