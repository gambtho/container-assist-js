/**
 * Verify Deployment Tool - TypeScript Implementation
 * Verifies Kubernetes deployment health and retrieves endpoints
 */

import { z } from 'zod'
import { Result, ok, fail, ErrorCode, DomainError } from '../../../domain/types/index.js'
import type { ToolHandler, ToolContext } from '../tool-types.js'

// Input schema
const VerifyDeploymentInput = z.object({
...
async function checkDeploymentHealth(
  ...
  namespace: string,
  context: ToolContext
): Promise<{ name: string; endpoint: string; status?: 'healthy' | 'unhealthy' | 'degraded'; response_time_ms?: number }> {
...
async function getPodInfo(
  ...
  deploymentName: string,
  context: ToolContext
): Promise<Array<{ name: string; ready: boolean; status: string; restarts?: number; node?: string }>> {
...
async function getServiceEndpoints(
  ...
  serviceName: string,
  context: ToolContext
): Promise<Array<{ service: string; type: string; url?: string; port?: number; external?: boolean }>> {
  const { kubernetesService, logger } = context
...
function analyzeIssues(
  ...
  for (const deployment of deployments) {
    ...
    if (deployment.replicas.ready < minReadyPods) {
      issues.push(
        `Deployment ${deployment.name}: Less than minimum ${minReadyPods} pods ready`
      )
    }
  }
 ...
 */
export const verifyDeploymentHandler: ToolHandler<VerifyInput, VerifyOutput> = {
  ...
  async execute(input: VerifyInput, context: ToolContext): Promise<Result<VerifyOutput>> {
    const { logger, sessionService, progressEmitter } = context
    const { sessionId, namespace, deployments, services, checkEndpoints, checkPods, minReadyPods } = input
    ...
    try {
      ...
      if (targetDeployments.length === 0 && sessionId && sessionService) {
        ...
        if (deploymentResult) {
          ...
          if (deploymentResult.deployment_name) {
            targetDeployments = [deploymentResult.deployment_name]
          }
          if (deploymentResult.service_name) {
      ...
      if (targetDeployments.length === 0 && targetServices.length === 0) {
        return fail(new DomainError(
          ...
          'No deployments or services to verify'
        ))
      }
      ...
      let podResults: Array<{ name: string; ready: boolean; status: string; restarts?: number; node?: string }> = []
      ...
      const healthy = deploymentResults.every(d => d.ready) &&
      ...
      if (progressEmitter && sessionId) {
        await progressEmitter.emit('workflow.step.progress', {
          ...
          message: healthy ? 'Deployment verified successfully' : `Deployment has ${issues.length} issues`,
          progress: 1.0
        })
      }
