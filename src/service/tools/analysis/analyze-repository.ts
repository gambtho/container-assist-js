/**
 * Repository Analysis Tool - TypeScript Implementation
 * Comprehensive analysis of repository structure, language, framework, and build system
 */
...
import { ErrorCode, DomainError } from '../../../domain/types/errors.js'
 ...
 */
async function validateRepositoryPath(repoPath: string): Promise<{ valid: boolean; error?: string }> {
  try {
    ...
    return { valid: false, error: `Cannot access repository: ${error}` }
  }
}
...
async function detectLanguage(repoPath: string): Promise<{ language: string; version?: string }> {
  ...
  for (const [lang, signature] of Object.entries(LANGUAGE_SIGNATURES)) {
    ...
    const hasExtensions = signature.extensions?.some(ext => (extensionCounts[ext] ?? 0) > 0) || false
    if (hasExtensions) {
      return { language: lang }
    }
  }
 ...
 * Detect framework
 */
async function detectFramework(repoPath: string, language: string): Promise<{ framework?: string; version?: string }> {
  const files = await fs.readdir(repoPath)
    ...
    try {
      ...
      for (const [framework, signature] of Object.entries(FRAMEWORK_SIGNATURES)) {
        if (signature.dependencies?.some(dep => dep in allDeps)) {
          ...
          return { framework, version: firstDep ? allDeps[firstDep] : undefined }
        }
      }
    } catch {}
  }
 ...
 * Detect build system
 */
async function detectBuildSystem(repoPath: string): Promise<typeof BUILD_SYSTEMS[keyof typeof BUILD_SYSTEMS] & { type: string } | undefined> {
  const files = await fs.readdir(repoPath)
 ...
 */
async function analyzeDependencies(repoPath: string, language: string): Promise<Array<{ name: string; version?: string; type?: 'runtime' | 'dev' | 'test'}>> {
 ...
 */
async function checkDockerFiles(repoPath: string): Promise<{ hasDockerfile: boolean; hasDockerCompose?: boolean; hasKubernetes?: boolean }> {
  const files = await fs.readdir(repoPath)
 ...
 */
export const analyzeRepositoryHandler: ToolHandler<AnalyzeInput, AnalyzeOutput> = {
  ...
  async execute(input: AnalyzeInput, context: ToolContext): Promise<Result<AnalyzeOutput>> {
 ...
 */
function getSecurityRecommendations(dependencies: Array<{ name: string; version?: string; type?: 'runtime' | 'dev' | 'test'}>): string[] {
  ...
  if (foundVulnerable.length > 0) {
    recommendations.push(`Update vulnerable packages: ${foundVulnerable.map(d => d.name).join(', ')}`)
  }
