/**
 * Tag Image Tool - TypeScript Implementation
 * Tags Docker images for registry organization
 */
...
const TagImageInput = z.object({
  ...
  targetTags: data.target_tags || data.targetTags ||
...
async function tagDockerImage(
  ...
  if (dockerClient && 'tag' in dockerClient) {
    const result = await (dockerClient as any).tag(source, target)
 ...
 */
export const tagImageHandler: ToolHandler<TagInput, TagOutput> = {
  name: 'tag_image',
  ...
  async execute(input: TagInput, context: ToolContext): Promise<Result<TagOutput>> {
    ...
    try {
      ...
      if (!source && sessionId && sessionService) {
        ...
        if (buildResult) {
          source = buildResult.image_id || buildResult.image_tag
        }
      ...
      if (registry) {
        allTags = allTags.map(tag =>
          tag.startsWith(registry) ? tag : `${registry}/${tag}`
        )
      }
      ...
      if (successfulTags.length === 0) {
        return fail(new DomainError(
          ...
          'No tags were successfully created'
        ))
      }
      ...
      if (sessionId && sessionService) {
        await sessionService.updateAtomic(sessionId, (session) => ({
          ...
          workflow_state: {
            ...
            tag_result: {
              tags: tagResults.map(t => t.fullTag),
              registry,
            }
          }
        }))
