/**
 * Push Image Tool - TypeScript Implementation
...
async function pushImage(
  ...
  const { dockerClient, logger } = context
  const startTime = Date.now()

  if (dockerClient && 'push' in dockerClient) {
    const result = await (dockerClient as any).push({
      image: tag,
      registry,
...
async function pushWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      logger.warn(`Push attempt ${attempt} failed`, { tag, error: lastError.message })
 ...
 */
export const pushImageHandler: ToolHandler<PushInput, PushOutput> = {
  name: 'push_image',
  ...
  async execute(input: PushInput, context: ToolContext): Promise<Result<PushOutput>> {
    const { logger, sessionService, progressEmitter } = context
    const { sessionId, tags, registry, username, password, authToken, retryOnFailure, parallel } = input
    ...
    try {
      ...
      if (imagesToPush.length === 0 && sessionId && sessionService) {
        ...
        if (session.workflow_state?.tag_result) {
          ...
          imagesToPush = [session.workflow_state.build_result.image_tag].filter(Boolean)
        }
      }
      ...
      const authenticated = await authenticateRegistry(
        targetRegistry,
        credentials,
        context
      )
      ...
      if (parallel) {
        ...
        const pushPromises = imagesToPush.map(async (tag) => {
          try {
            ...
            logger.error(`Failed to push ${tag}`, { error: errorMessage })
          }
        })
      ...
      const totalPushTime = pushed.reduce((sum, p) => sum + p.pushTime, 0)
      ...
      if (progressEmitter && sessionId) {
        progressEmitter.emit('workflow.step.failed', {
          step: 'push_image',
