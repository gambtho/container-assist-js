...
import { Session, WorkflowState, Logger, ServiceError, ErrorCode } from '../../domain/types/index.js'
import { SessionStore, SessionFilter } from '../../domain/types/session-store.js'
import { SessionUtils } from './utils.js'
...
export class SessionService extends EventEmitter {
    ...
    try {
      ...
      if (parsed.sessions && Array.isArray(parsed.sessions)) {
        ...
        const validSessions = parsed.sessions.filter(
          (s: Session) => !SessionUtils.isExpired(s)
        )
   ...
   */
  async create(data: { projectName?: string; metadata?: any }): Promise<Session> {
  ...
  async createSession(repoPath: string, data?: Partial<Session>): Promise<Session> {
    ...
    if (activeCount >= this.config.maxActiveSessions) {
      throw new ServiceError(ErrorCode.ResourceLimitExceeded, `Maximum active sessions (${this.config.maxActiveSessions}) reached`)
    }

    const session = SessionUtils.createSession(repoPath, {
      ...data,
      expires_at: data?.expires_at || new Date(
        Date.now() + this.config.defaultTTL * MILLISECONDS_PER_SECOND
      ).toISOString(),
    })
  ...
  async updateWorkflowState(
    ...
    await this.store.updateAtomic(id, (current) => ({
      ...current,
      workflow_state: SessionUtils.mergeWorkflowState(
  ...
  async markStepCompleted(id: string, step: string): Promise<Session> {
    await this.store.updateAtomic(id, (current) =>
      SessionUtils.markStepCompleted(current, step)
    )
  ...
  async setCurrentStep(id: string, step: string | null): Promise<Session> {
    await this.store.updateAtomic(id, (current) =>
      SessionUtils.setCurrentStep(current, step)
    )
  ...
  async addStepError(id: string, step: string, error: Error | string): Promise<Session> {
    await this.store.updateAtomic(id, (current) =>
      SessionUtils.addStepError(current, step, error)
    )
  ...
  async completeSession(id: string, success: boolean = true): Promise<Session> {
    return this.updateSession(id, {
      ...
      expires_at: new Date(Date.now() + COMPLETED_SESSION_RETENTION * MILLISECONDS_PER_SECOND).toISOString(),
    })
  }
  ...
  async extendSession(id: string, additionalSeconds: number): Promise<Session> {
    await this.store.updateAtomic(id, (current) => {
      ...
      return {
        ...current,
        expires_at: new Date(currentExpiry + additionalSeconds * MILLISECONDS_PER_SECOND).toISOString(),
      }
    })
  ...
  async getSessionMetrics(): Promise<{
    ...
    const [active, completed, failed, expired, pending, analyzing, building, deploying] = await Promise.all([
    ...
    return {
      total: active.length + completed.length + failed.length + expired.length +
             pending.length + analyzing.length + building.length + deploying.length,
