 ...
 */
export const fixDockerfileHandler: MCPToolDescriptor<
  FixDockerfileInputType,
  FixDockerfileOutputType
> = {
  name: 'fix-dockerfile',
  description:
  ...
  handler: async (
    ...
    return await withRetry(
      async () => {
        ...
        if (!dockerfileContent ?? input.sessionId) {
        ...
        const fix = await (context as unknown).structuredSampler.sampleJSON(
          {
            ...
            variables: {
              ...
              language: repositoryAnalysis?.language ?? 'unknown',
              framework: repositoryAnalysis?.framework ?? 'unknown',
              dependencies: JSON.stringify(repositoryAnalysis?.dependencies ?? []),
              build_system: repositoryAnalysis?.build_system?.type ?? 'unknown',
              entry_point: repositoryAnalysis?.entryPoint ?? 'unknown'
            }
          },
          DockerfileFixSchema
        );
        ...
        if (!fix.success) {
          throw new Error(`AI Dockerfile fixing failed: ${fix.error?.message}`);
        }
        ...
        if (context.contentValidator?.validateContent) {
        ...
        if (input.sessionId && context.sessionService) {
          try {
            await context.sessionService.updateAtomic(input.sessionId, (session: unknown) => ({
              ...session,
              workflowState: {
...
function validateDockerfileFix(fix: DockerfileFix): ValidationResult {
  ...
  if (!fix.fixed_dockerfile?.trim()) {
    issues.push('Fixed Dockerfile is empty');
  }

  if (!fix.root_cause_analysis?.trim()) {
  ...
  if (fix.fixed_dockerfile?.includes(':latest')) {
