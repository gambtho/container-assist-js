...
import { Session, SessionSchema } from '../../contracts/types/session';
import {
  SessionStore,
  SessionFilter,
  SessionNotFoundError,
  SessionAlreadyExistsError
} from '../../contracts/types/session-store';
import { SessionUtils } from '../../application/session/shared';
...
export class InMemorySessionStore implements SessionStore {
  ...
  private startCleanupTimer(): void {
    ...
    this.cleanupInterval = setInterval(() => {
      this.deleteExpired().catch((err) => this.logger.error({ error: err }, 'Cleanup failed'));
  ...
  async updateAtomic(id: string, updater: (current: Session) => Session): Promise<void> {
    ...
    try {
      ...
      if (updated.workflow_state) {
        const progress = { current: 0, total: 100 }; // Default progress
        updated.progress = {
  ...
  async list(filter?: SessionFilter): Promise<Session[]> {
    ...
    if (filter) {
      ...
      if (filter.createdBefore) {
        sessions = sessions.filter((s) => new Date(s.created_at) < filter.createdBefore!);
      }
  ...
  async getActiveCount(): Promise<number> {
    return Array.from(this.sessions.values()).filter(
      (s) => s.status === 'active' && !SessionUtils.isExpired(s)
    ).length;
  }
