...
import { SessionStore } from '../../contracts/types/session-store';
...
export class SessionStoreFactory {
   ...
   */
  static async create(config: StoreConfig, logger: Logger): Promise<SessionStore> {
    logger.info({ storeType: config.type }); // Fixed logger call

    if (config.type === 'memory') {
      // Cast domain Logger to pino Logger for compatibility
      return new InMemorySessionStore(logger as unknown);
    }
