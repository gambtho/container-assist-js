...
export const createIntegrationFlowTests = (testRunner: MCPTestRunner): TestCase[] => {
  ...
  const tests: TestCase[] = [
    ...
    {
      name: 'deployment-preparation-flow',
      category: 'integration-flows',
      description: 'Test deployment preparation workflow',
      tags: ['integration', 'deployment', 'kubernetes'],
      timeout: 90000,
      execute: async () => {
        const start = performance.now();
        const sessionId = 'deployment-prep';
        ...
        try {
          ...
          steps.push({
            step: 'cluster-preparation',
            success: !prepareResult.isError,
            error: prepareResult.error?.message
          });

          // Step 2: Generate K8s Manifests
          const manifestsResult = await client.callTool({
            name: 'generate-k8s-manifests',
            arguments: {
              sessionId,
          ...
          steps.push({
            ...
            success: !manifestsResult.isError,
            error: manifestsResult.error?.message
          });
          ...
          const successfulSteps = steps.filter(s => s.success).length;
          const totalSteps = steps.length;
          const workflowSuccess = successfulSteps >= 2; // At least 2 steps should work

          return {
            success: workflowSuccess,
            duration: responseTime,
            message: `Deployment preparation workflow: ${successfulSteps}/${totalSteps} steps successful`,
            details: {
              steps,
              successfulSteps,
              totalSteps,
              workflowType: 'deployment-preparation'
            },
            performance: {
              responseTime,
              memoryUsage: 0,
          ...
          return {
            success: false,
            duration: performance.now() - start,
            message: `Deployment workflow failed: ${error instanceof Error ? error.message : 'Unknown error'}`
          };
        }
      }
    },
