...
describe('CacheManager', () => {
  ...
  describe('cache maintenance', () => {
    ...
    it('should cleanup expired entries', async () => {
      const shortTtlCache = new CacheManager(cacheDir, { ttl: 50 }, mockLogger);
      await shortTtlCache.initialize();

      await shortTtlCache.set('expire-key', { data: 'will expire' });
      await shortTtlCache.set('keep-key', { data: 'will keep' });
    ...
    it('should handle cleanup errors gracefully', async () => {
      ...
      // Should still succeed even if some files can't be deleted
      expect(cleanupResult.ok).toBe(true);
